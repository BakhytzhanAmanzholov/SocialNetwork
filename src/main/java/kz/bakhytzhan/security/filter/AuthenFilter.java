...
public class AuthenFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws IOException, ServletException {
        ...
        String access_token = JWT.create().withSubject(user.getUsername()).withExpiresAt(new Date(System.currentTimeMillis() + 15 * 60 * 1000))
...
//        response.setHeader("refresh_token",refresh_token);
        ...
        acc_token.setMaxAge(15 * 60 * 1000); // expires in 7 days
        acc_token.setHttpOnly(true);
        Cookie ref_token = new Cookie("refresh_token",refresh_token );
        ref_token.setMaxAge(15 * 60 * 1000); // expires in 7 days
